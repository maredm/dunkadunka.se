...
exports.read = read;
exports.convertToIXML = convertToIXML;
exports.convertIXMLtoObject = convertIXMLtoObject;
function download(samples, sampleRate = 48000, name = 'output', bext, ixml) {
    const channels = 1;
    ...
    if (bext) {
        ...
        if (version > 0) {
            if (umidBytes) {
                for (let i = 0; i < 64; i++)
                    bextPayload[p++] = i < umidBytes.length ? umidBytes[i] : 0;
            }
            else {
                for (let i = 0; i < 64; i++)
                    bextPayload[p++] = 0;
        ...
        if (codingHistory) {
            for (let i = 0; i < codingHistory.length; i++)
                bextPayload[p++] = codingHistory.charCodeAt(i) & 0xff;
            bextPayload[p++] = 0;
        }
        // pad if needed
        while (p < bextDataSize)
            bextPayload[p++] = 0;
    }
    ...
    if (ixml) {
        ...
        if (enc)
            ixmlPayload = enc.encode(ixml);
        else {
            const arr = new Uint8Array(ixml.length);
            for (let i = 0; i < ixml.length; i++)
                arr[i] = ixml.charCodeAt(i) & 0xff;
            ixmlPayload = arr;
        }
    ...
    if (ixmlPayload) {
        ...
        if (ixmlDataSize > ixmlPayload.length) {
            for (let i = ixmlPayload.length; i < ixmlDataSize; i++)
                dest[i] = 0;
        }
        ptr += ixmlDataSize;
        if (ixmlDataSize % 2 === 1) {
...
function read(bufferOrBlob) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (bufferOrBlob instanceof Blob)
            buffer = yield bufferOrBlob.arrayBuffer();
        ...
        while (offset + 8 <= view.byteLength) {
            ...
            if (id === 'fmt ') {
                ...
                const readLen = (n) => Math.max(0, Math.min(n, (chunkStart + size) - p));
                const safeRead = (n) => {
                    const r = readLen(n);
...
function convertIXMLtoObject(xmlString) {
    return xmlString;
}
