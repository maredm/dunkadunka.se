...
exports.createFIRFromFFT = createFIRFromFFT;
exports.firToMinPhase = firToMinPhase;
exports.createInverseFIRFromFFT = createInverseFIRFromFFT;
...
class IIRFilter {
    ...
    get coefficients() {
        return createBiquadCoefficients(this.type, this.freq, this.Q, this.gain, this.sampleRate);
    }
    getFrequencyResponse(fftSize = 8192) {
        ...
        const frequencies = (0, math_js_1.linspace)(0, 48000 / 2, fftSize);
 ...
 */
function createBiquadCoefficients(type = 'peaking', freq = 1000, Q = 1, gain = 0, // in dB (used by peaking and shelving)
 ...
 */
function createWebAudioBiquad(audioCtx, { type = 'peaking', freq = 1000, Q = 1, gain = 0 } = {}) {
 ...
 */
function createFIRFromFFT({ target, reference = null, fftSize = null, taps = 513, kirkebyEps = 1e-6, window = 'hann' } = {}) {
 ...
 */
function createInverseFIRFromFFT({ reference, fftSize = null, taps = 513, kirkebyEps = 1e-6, window = 'hann' } = {}) {
    const N = fftSize || (reference && reference.re && reference.re.length);
    if (!reference || !reference.re)
        throw new Error('reference required for inverse FIR');
