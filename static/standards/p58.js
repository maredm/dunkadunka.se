...
exports.SpeechVoltmeter = void 0;
exports.initSpeechVoltmeter = initSpeechVoltmeter;
exports.speechVoltmeter = speechVoltmeter;
/**
...
function speechVoltmeter(buffer, state) {
    ...
    for (let k = 0; k < smpno; k++) {
        ...
        state.intermediateQuantity = (g * state.intermediateQuantity + (1 - g) * absX);
        state.envelope = (g * state.envelope + (1 - g) * state.intermediateQuantity);
    ...
    const longTermLevel = 10 * Math.log10(state.samplesSquaredSum / state.samplesProcessedCount + MIN_LOG_OFFSET);
    ...
    let AdB = 10 * Math.log10(state.samplesSquaredSum / state.activity[0] + MIN_LOG_OFFSET);
    ...
    for (let j = 1; j < THRESHOLD_COUNT; j++) {
        if (state.activity[j] !== 0) {
            AdB = 10 * Math.log10(state.samplesSquaredSum / state.activity[j] + MIN_LOG_OFFSET);
            ...
            if (delta <= MARGIN) {
                ...
                const AmdB = 10 * Math.log10(state.samplesSquaredSum /
                    state.activity[j - 1] + MIN_LOG_OFFSET);
                const CmdB = 20 * Math.log10(state.threshold[j - 1] + MIN_LOG_OFFSET);
                activeSpeechLevel = binInterp(AdB, AmdB, CdB, CmdB, MARGIN, 0.5);
...
class SpeechVoltmeter {
     ...
     */
    reset(sampleRate) {
        const rate = sampleRate !== null && sampleRate !== void 0 ? sampleRate : this.state.fs;
