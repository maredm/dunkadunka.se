/**
 * /c:/Users/edmark/Documents/projekt_dunka_dunka/static/modules/filter.js
 *
...
function clampFreq(freq, sr) {
    const nyq = sr / 2;
    return Math.max(0, Math.min(freq, nyq - 1e-6));
}

export class IIRFilter {
    constructor(freq, Q, gain, sampleRate, type) {
        this.z1 = 0;
        this.z2 = 0;
    ...
    get coefficients() {
        return createBiquadCoefficients(
            this.type, this.freq, this.Q, this.gain, this.sampleRate
        );
    }
    
    getFrequencyResponse(fftSize = 8192) {
        ...
        // vertical freq grid (decades)
        ctx.fillStyle = 'rgba(255,255,255,0.7)';
        ctx.strokeStyle = 'rgba(255,255,255,0.08)';
        const decades = [];
        const startDec = Math.floor(Math.log10(minFreq));
        const endDec = Math.ceil(Math.log10(maxFreq));
        for (let d = startDec; d <= endDec; d++) decades.push(Math.pow(10, d));
        for (const f of decades) {
            ...
            ctx.stroke();
            ctx.fillText((f >= 1000 ? (f / 1000) + 'k' : f), x - 10, height - 4);
        }

        // draw 0 dB line more prominently
        if (minDb <= 0 && maxDb >= 0) {
            const y0 = yForDb(0);
        ...
        for (let i = 0; i < bins; i++) {
            const f = freqForX(i);
            const x = xForFreq(f);
            const y = yForDb(magsDb[i]);
            if (i === 0) ctx.moveTo(x, y);
            else ctx.lineTo(x, y);
        }
        ctx.stroke();

        // draw title / parameters
        ctx.fillStyle = 'rgba(255,255,255,0.9)';
        ctx.font = '13px sans-serif';
        const title = `${this.type} — f=${Math.round(this.freq)}Hz Q=${this.Q} gain=${this.gain}dB`;
        ctx.fillText(title, 50, 14);
 ...
 * @param {string} options.type - Filter type (see above)
 * @param {number} options.freq - Center/cutoff frequency in Hz
 * @param {number} options.Q - Quality factor (resonance)
 * @param {number} options.gain - Gain in dB (for peaking and shelving filters)
...
function createBiquadCoefficients(
    ...
    freq = 1000,
    Q = 1,
    gain = 0, // in dB (used by peaking and shelving)
    ...
    switch (type) {
        ...
        case 'gain':
            a0 = 1; a1 = 0; a2 = 0; b0 = gain; b1 = 0; b2 = 0;
            break;
 ...
 */
function createWebAudioBiquad(audioCtx, {
    type = 'peaking',
    freq = 1000,
    Q = 1,
    gain = 0
...
export function createFIRFromFFT({
    ...
    const pad = (arr) => {
        ...
        out.set(arr.subarray ? arr.subarray(0, Math.min(arr.length, N)) : arr.slice(0, Math.min(arr.length, N)));
        return out;
    };
    const Tre = pad(tRe);
    const Tim = pad(tIm);

    let Gre = null, Gim = null;
    ...
    if (Gre) {
        for (let k = 0; k < N; k++) {
            const tr = Tre[k], ti = Tim[k];
            const gr = Gre[k], gi = Gim[k];
...
export function firToMinPhase(input, { fftSize = null, eps = 1e-12 } = {}) {
    ...
    fft.completeSpectrum && fft.completeSpectrum(Xc);
    ...
    fft.completeSpectrum && fft.completeSpectrum(Sc);
