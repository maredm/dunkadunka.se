...
import { fft } from "../src/signal/fft";

function expectArrayClose(actual: ArrayLike<number>, expected: number[], precision = 10): void {
	const actualValues = Array.from(actual);
	expect(actualValues.length).toBe(expected.length);
	for (let index = 0; index < expected.length; index += 1) {
		expect(actualValues[index]).toBeCloseTo(expected[index], precision);
	}
}
...
test("fft accepts complex-number input", () => {
	const { real, imag } = fft({ real: [1, 0, 0, 0], imag: [0, 0, 0, 0] });
